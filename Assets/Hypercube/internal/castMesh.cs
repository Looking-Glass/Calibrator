    ...
    public class castMesh : MonoBehaviour
    {
     ...
     //   public Material testMaterial2 = null;
        ...
        public void saveConfigSettings()
        {
            ...
            bool hasPath = hypercube.utils.getConfigPath(relativeSettingsPath, out d.fileName);    //return it to the dataFileDict as an absolute path within that drive if we find it  (ie   G:/volumeConfigurationData/prefs.txt).
            ...
            //version 2 of calibration stuff
            if (hasPath) //we know we have a place we can save to
            {
        ...
        public void saveVertPositions(int w, int h, string savePath)
        {
        ...
        public void getScreenResolution(ref int w, ref int h)
        {
        ...
        public void updateMesh()
        {
            ...
            for (int s = 0; s < slices; s++)
            {
                ...
                vertCount += generateSliceShard(topL, topM, midL, middle, UV_ul, UV_mid, bows[s], shardOrientation.UL, vertCount, ref verts, ref tris, ref uvs, ref colors, (float)s *.001f); //top left shard
                vertCount += generateSliceShard(topM, topR, middle, midR, UV_top, UV_right, bows[s], shardOrientation.UR, vertCount, ref verts, ref tris, ref uvs, ref colors, (float)s * .001f); //top right shard
                vertCount += generateSliceShard(midL, middle, lowL, lowM, UV_left, UV_bottom, bows[s], shardOrientation.LL, vertCount, ref verts, ref tris, ref uvs, ref colors, (float)s * .001f); //bottom left shard
                vertCount += generateSliceShard(middle, midR, lowM, lowR, UV_mid, UV_br,  bows[s], shardOrientation.LR, vertCount, ref verts, ref tris, ref uvs, ref colors, (float)s * .001f); //bottom right shard
        ...
        int generateSliceShard(Vector2 topLeft, Vector2 topRight, Vector2 bottomLeft, Vector2 bottomRight, Vector2 topLeftUV, Vector2 bottomRightUV, Vector4 bow, shardOrientation o, int startingVert, ref  List<Vector3> verts, ref List<int> triangles, ref List<Vector2> uvs, ref List<Color> colors, float slice)
        {
            int vertCount = 0;
            ...
            for (var i = 0; i <= tesselation; i++)
            {
                for (var j = 0; j <= tesselation; j++)
                {
                    ...
                    colors.Add( new Color( xLerp, yLerp, slice, 1f)); //note the current slice is stored in the blue channel
                }
            }
        ...
        public void generateSullyTextures(int w, int h, string filePath)
        {
            ...
            c.targetTexture = rtt;
            c.RenderWithShader(screenShotShader, "");
        ...
        //this method maps a float into a rgb 24 bit color space
